...
import {applyTEnv, globalEnvAddTBinding, makeExtendTEnv, TEnv, theGlobalTEnv} from "./TEnv";
...
export const L5typeof = (concreteExp: string): string | Error =>
    unparseTExp(typeofExp(parse(concreteExp), theGlobalTEnv));
...
// Typing rule:
//       type<define.var.var>(tenv) = t1
//       type<define.val>(extend-tenv(define.var1.var = t1)) = t
// if t1 = t
// then typeOf< define (var : texp) val >(tenv) = voidTExp
// TODO - write the true definition
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    const constraint1 = checkEqualType(exp.var.texp,typeofExp(exp.val, tenv), exp);
        if (isError(constraint1)) {
            return constraint1;
        }
        else{
