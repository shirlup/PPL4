...
import {isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, PairTExp} from './TExp';
...
//         |  ( Pair <cexp> <cexp>)           / PairExp(left: CExp , right: CExp)
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | PairExp;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isPairExp(x);
export const expComponents = (e: Exp): CExp[] =>
    isIfExp(e) ? [e.test, e.then, e.alt] :
    isPairExp(e)? [e.left , e.right]:
    isProcExp(e) ? e.body :
...
export interface PairExp {tag: "PairExp"; left:CExp, right: CExp;};
export const makePairExp = (left: CExp, right: CExp): PairExp =>
    ({tag: "PairExp", left: left , right: right});
export const isPairExp = (x: any): x is PairExp => x.tag === "PairExp";
...
const parseCompoundCExp = (sexps: any[]): CExp | Error =>
    ...
    first(sexps) === "'" ? parseLitExp(sexps) :
    first(sexps) == "cons" ? parsePairExp(sexps):
    parseAppExp(sexps)

const parsePairExp = (sexps: any[]): PairExp| Error =>
    sexps.length != 3 ? Error(" Pair is constructed of two elements"):
    safeFL((cexps: CExp[]) => makePairExp(first(cexps), second(cexps)))(map(parseCExp, rest(sexps)));
...
export const parseLitExp = (sexps: any[]): LitExp| PairExp | Error => {
    const pairCheck = second(sexps);
    if(isArray(pairCheck) && pairCheck.indexOf('.') === 1 && pairCheck.length === 3){
        const newPair = ["Pair",pairCheck[0],pairCheck[2]];
        return parsePairExp(newPair);
    }
    return safeF(makeLitExp)(parseSExp(second(sexps)));
}
...
export const unparse = (e: Parsed | Error): string | Error =>
    ...
    isPairExp(e) ? `(cons ${unparse(e.left)} ${unparse(e.right)} )`:
