...
*/
const parseCompoundTExp = (texps: any[]): LitTExp|PairTExp|ProcTExp | Error => {
    if(first(texps) == "Pair")
        return parsePairTExp(rest(texps));
...
const parsePairTExp = (texps: any[]): PairTExp | Error => {
    if(texps.length != 2)
        return Error('Pair is composed of two elements only');
    return safeMakePairTExp(parseTExp(texps[0]),parseTExp(texps[1]));
}
...
const safeMakePairTExp = (left: TExp|Error, right: TExp|Error): Error | PairTExp =>
    isError(left) ? left :
        isError(right) ? right :
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>
        ...
        isPairTExp(x) ? ['Pair', unparseTExp(x.left), unparseTExp(x.right)]    :
        ["never"];
...
export const matchTVarsInTE = <T1, T2>(te1: TExp, te2: TExp, succ: (mapping: Array<Pair<TVar, TVar>>) => T1, fail: () => T2): T1 | T2 =>
    ...
    (isPairTExp(te1) && isPairTExp(te2)) ? matchTVarsInTPairs(te1,te2,succ,fail):
    matchTVarsInTProcs(te1, te2, succ, fail);

const matchTVarsInTPairs = <T1, T2>(te1: PairTExp, te2: PairTExp, succ: (mapping: Array<Pair<TVar, TVar>>) => T1, fail: () => T2): T1 | T2 =>
    matchTVarsInTEs(pairTExpComponents(te1), pairTExpComponents(te2), succ, fail) ;

// te1 and te2 are the result of tvarDeref
const matchTVarsinTVars = <T1, T2>(te1: TExp, te2: TExp, succ: (mapping: Array<Pair<TVar, TVar>>) => T1, fail: () => T2): T1 | T2 =>
